...
describe('validate', () => {
	...
	it('should validate a string value successfully', () => {
		vi.mocked(is).mockReturnValue(true);

		const errors = validate('Hello, World!', { type: 'String' });
	...
	it('should return an error for an invalid field type', () => {
		...
		expect(vi.mocked(validationError.fromString)).toBeCalledWith('Expected string field');
