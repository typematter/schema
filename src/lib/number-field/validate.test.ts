...
describe('validate', () => {
	...
	it('should validate a number value successfully', () => {
	...
	it('should return an error for an invalid field type', () => {
		...
		expect(vi.mocked(validationError.fromString)).toBeCalledWith('Expected number field');

		expect(errors.length).toBe(1);
		expect(errors[0]).toBe('mock error');
	});

	it('should return an error for a non-number value', () => {
		vi.mocked(is).mockReturnValue(true);

		const errors = validate('Hello, World', { type: 'Number' });
