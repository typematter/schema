...
describe('validate', () => {
	...
	it('should return an error for an invalid field type', () => {
		...
		expect(vi.mocked(validationError.fromString)).toBeCalledWith('Expected boolean field');

		expect(errors.length).toBe(1);
		expect(errors[0]).toBe('mock error');
	});

	it('should return an error for a non-boolean value', () => {
		vi.mocked(is).mockReturnValue(true);

		const errors = validate(42, { type: 'Boolean' });

		expect(vi.mocked(validationError.fromString)).toBeCalledWith(
			'Expected boolean value, got number'
		);
